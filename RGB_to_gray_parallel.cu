...
void  parallel_RGB_gray(unsigned char *img ,unsigned char * gray,int width,int height) {
    ...
    //call kernel
    dim3 numThreadsPerBlock(32,32,1); // 32 x 32
    ...
    rgb_to_gray_kernel<<<no_blocks, no_thread_per_block>>>(red_d, green_d, blue_d, gray_d, width, height);
...
int main(){
    ...
    unsigned char *img_gray_parallel = new unsigned char[width * height];// channel is 1

    parallel_RGB_gray(img,img_gray_parallel,width,height);

    stbi_write_png("parallel_gray.png", width, height, 1 , img_gray_parallel, width);
